...
import com.huoyi.photovault.util.AppForegroundState
...
class BackgroundScanWorker @AssistedInject constructor(
    ...
    companion object {
        ...
        fun applyScanInterval(context: Context, intervalValue: Int) {
            if (intervalValue == com.huoyi.photovault.data.local.SettingsPreferences.SCAN_INTERVAL_TEST_10S) {
    ...
    private suspend fun doWorkForActiveSession(): Result {
        ...
        if (inputData.getBoolean(KEY_RESUME_PAUSED, false)) {
            return try {
                ...
                com.huoyi.photovault.util.FileLogger.log("Scan", "resume run: resume() dispatched")
                Result.success()
            } catch (e: Exception) {
                com.huoyi.photovault.util.FileLogger.log(
                    "Scan",
        ...
        if (isTestRun &&
            ...
            com.huoyi.photovault.data.local.SettingsPreferences.SCAN_INTERVAL_TEST_10S
        ) {
    ...
    /**
     * Re-enqueues files that have a persisted, still-valid [com.huoyi.photovault.data.local.entity.UploadRecord]
