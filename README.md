...
服务端会在 PhotoVault SQLite 数据库的 `server_metadata` 表中持久化一个 UUID `instance_id`，并在登录、注册和刷新令牌响应中同时返回当前账户的数据库主键 `user_id`。Android 只用稳定的 **`(instance_id, user_id)`** 作为备份会话身份；服务器地址只是当前请求的路由，不再用于判断是不是同一服务器。

| 登录场景 | 客户端行为 |
| --- | --- |
| 同一 `instance_id`、同一 `user_id` 重新登录 | 仅原子更新服务器地址、记住密码设置、稳定身份和令牌；保留文件备份状态、断点、队列、历史、文件夹统计和扫描水位，不触发额外全量扫描 |
| 同一实例更换 IP、域名、端口、基础路径或 HTTP/HTTPS | 仍是同一会话；行为与同一用户重新登录相同，不清理状态，也不会因为地址变化重新上传 |
| 登录不同 `instance_id` 的服务器 | 停止旧会话的扫描与上传；清除旧实例的文件状态、上传断点、待上传队列和备份历史；保留已选择的本地文件夹、SAF 授权与存储策略；重置文件夹统计后执行全量扫描 |
| 同一实例登录不同 `user_id` | 按新会话处理，与切换实例相同，防止旧账户的「已备份」状态和任务被新账户继承 |

切换到新的稳定身份后：

- 客户端先取消旧 WorkManager 扫描，等待前台上传和状态同步退出，并清空持久队列，避免旧任务跨会话继续执行。
- 客户端清除 `photo_status`、上传断点、持久队列和备份历史，但保留文件夹选择、SAF 授权、存储策略以及「自动备份」设置。
- 「自动备份」开启时，全量扫描会向新账户重新执行 SHA-256 查重；服务端已有文件跳过传输，缺失文件重新上传。
- 「自动备份」关闭时，只重新扫描并刷新当前会话的状态和计数；点击「立即备份」后才会查重和上传。
- 服务器地址和令牌通过同一个不可变请求快照使用，避免切换瞬间把新令牌发送到旧地址。

兼容规则：

- 从旧版 Android 升级时，本地尚无稳定身份。首次成功连接新版服务端会保守地执行一次会话重置和全量扫描；稳定身份保存后，后续退出登录只清除令牌并保留 `instance_id/user_id`，不会重复重置。
- 旧版服务端若未返回 `instance_id` 或 `user_id`，Android 会拒绝发布该登录/刷新会话并提示升级服务端，避免退回到不可靠的地址或用户名猜测。
- 刷新令牌若返回缺失或不同的稳定身份，客户端会清除令牌并要求交互式重新登录，不会静默切换账户。
- `instance_id` 随 SQLite 数据库一起备份和恢复：同一数据库迁移到新机器后仍视为同一逻辑实例；全新数据库会生成不同 UUID。
